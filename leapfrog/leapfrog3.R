...
# Setup
  ...
  load("coords.RData")
    ...
    #########################################################
      ...
      game.map <- 1 # Pick which map to use [1,13]
      game.max <- 100 # Total number of games played
      game.settings.recommended <- TRUE # Defaults to recommended settings based on node count
      
      game.players <- 990 # Number of players [1, number of nodes]
      game.accuracy <- 1 # Starting accuracy of the players (0,1]
      game.length <- 2 # Iterations in each round
      game.players.loss <- FALSE # dynamic player count
      game.players.loss.rate <- 8 # percent fewer players each round
...
# Initialization
      ...
      plot.coords <- cbind(rep(1,node.count),tour,tour.x,tour.y)
      plot.coords <- rbind(plot.coords,plot.coords[1,])
      
      if(game.settings.recommended == TRUE){
        game.players <- round(node.count/2)
        game.accuracy <- round(node.count*.075)
...
# Update stats
      ...
      if (game.iter == 1){
        ...
        if(game.players.loss == TRUE){
          p <- (p-(game.players/(game.length*game.players.loss.rate)))
        } else {
          p <- game.players
        }
      }
...
# Jump
      jumpers <- sample(2:node.count, size=ceiling(p)) # Choose the jumpers
...
# Land
      for (i in 1:ceiling(p)){
        ...
        if(game.iter==1){
          place.size <- game.accuracy # Landing accuracy
        } else {
...
# Check round length and game length
      ...
      tour.x <- course.coords$X[match(tour,course.coords$Node)]
      tour.y <- course.coords$Y[match(tour,course.coords$Node)]
      plot.coords.new <- cbind(rep((iter+1),node.count),tour,tour.x,tour.y)
      plot.coords <- rbind(plot.coords,plot.coords.new)
      plot.coords <- rbind(plot.coords,plot.coords[(dim(plot.coords)[1]-node.count+1),])
